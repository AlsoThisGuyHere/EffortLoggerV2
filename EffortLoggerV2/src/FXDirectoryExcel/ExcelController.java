...
public class ExcelController {
	...
	public static List<EffortLog> readEffortLogs(File file)
	{
		try
		{
			...
			List<EffortLog> output = new ArrayList<EffortLog>();
			for(int i = 0; i < numEntries; i++)
			{
				Row row = sheet.getRow(i+2);
				int number = (int) row.getCell(0).getNumericCellValue();
				String date = row.getCell(1).getStringCellValue();
				String start = row.getCell(2).getStringCellValue();			// figure out if this and the next two can be dateTimeObjects
	...
	// Excel file format must be exact in order for method to work
	public static List<DefectLog> readDefectLogs(File file)
	{
		try
		{
			FileInputStream fis = new FileInputStream(file);
			...
			List<DefectLog> output = new ArrayList<DefectLog>();
			for(int i = 0; i < numEntries; i++)
			{
				Row row = sheet.getRow(i+2);
